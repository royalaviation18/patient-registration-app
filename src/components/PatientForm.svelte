<script>
    ...
    function validateForm() {
      ...
      if (!emergencyContactPhone || emergencyContactPhone.length !== 10) {
        emergencyContactPhoneError = 'Emergency contact phone must be exactly 10 digits';
      }
  
      if (!emergencyContactRelation.trim()) {
        emergencyContactRelationError = 'Emergency contact relation is required';
      }
  
      return !(nameError || dobError || ageError || genderError || maritalStatusError || phoneError || emailError || emergencyContactNameError || emergencyContactPhoneError || emergencyContactRelationError);
    }
  ...
  <form on:submit|preventDefault={register} class="form-container">
    <div class="form-field">
      ...
      <input type="text" id="name" bind:value={name} on:input={handleNameInput} />
    ...
    <div class="form-field">
      ...
      {#if maritalStatusError}<div class="error">{maritalStatusError}</div>{/if}
    </div>
  
    <div class="form-field">
      <label for="phone">Phone Number:</label>
      <input type="tel" id="phone" bind:value={phone} on:input={handlePhoneInput} />
    ...
    <div class="form-field">
      ...
      <input type="text" id="emergencyContactName" bind:value={emergencyContactName} />
      {#if emergencyContactNameError}<div class="error">{emergencyContactNameError}</div>{/if}
    </div>
    ...
    <div class="form-field">
      ...
      />
      {#if emergencyContactPhoneError}<div class="error">{emergencyContactPhoneError}</div>{/if}
    </div>
  
    <div class="form-field">
      <label for="emergencyContactRelation">Emergency Contact Relation:</label>
      <input type="text" id="emergencyContactRelation" bind:value={emergencyContactRelation} />
      {#if emergencyContactRelationError}<div class="error">{emergencyContactRelationError}</div>{/if}
    </div>
